...
def test_all(base_path, tests, test_method, visualize=False):
    assert test_method in [
        'kmeans', 'gmm', 'all'], f"Only support methods: {['kmeans', 'gmm', 'all']}. Got: {test_method}"
    run_experiment = partial(
        run_test, visualize=visualize, test_method=test_method)
    for test in tests:
    ...
    if visualize:
        ...
        for cluster_i in range(K):
            ...
            ax.scatter(test_data['data'][:, 0],
                       test_data['data'][:, 1])
            ax.scatter(test_data["gmm_centers"][:, 0],
                       test_data["gmm_centers"][:, 1], c="black")
            ax.scatter(test_data["kmeans_centers"][:, 0],
                       test_data["kmeans_centers"][:, 1], c="black", marker="x")
        ...
        if kmeans_enabled:
            for cluster_i in range(K):
                ...
                ax.scatter(test_data["kmeans_centers"][:, 0],
                           test_data["kmeans_centers"][:, 1], c="black")
...
def test_gmm(test_data):
    ...
    assert np.allclose(model.mixture_proportions,
                       test_data["gmm_mixture_proportions"])
