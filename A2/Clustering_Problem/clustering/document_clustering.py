...
def main(seed, num_trials, center_init, K, norm_flag, diffuse, max_iterations=1000):
    ...
    assert center_init in (
        "kmeans_pp", "random"), f"Support only kmeans_pp and random. Got: {center_init}"
    ...
    for trial_i in range(num_trials):
        ...
        mean_centers = mean_centers / \
            np.sum(mean_centers, axis=1, keepdims=True)
        ...
        for cluster_i in range(K):
            ...
            # Compute error
            total_error += np.sum(np.linalg.norm(
                data[labels.flatten() == cluster_i] - model.centers[cluster_i], ord=2, axis=1))
